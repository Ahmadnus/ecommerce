...
/*
|--------------------------------------------------------------------------
| TEMPORARY diagnostic — session / cookie troubleshooting
|--------------------------------------------------------------------------
| Guarded by a secret path segment. DELETE THIS BLOCK once /adlogin works.
| Reports why the server is not issuing a session cookie.
*/
Route::get('/__diag/fc5a9d49c79c3458', function (\Illuminate\Http\Request $request) {
    $sessionPath = storage_path('framework/sessions');

    $sessionsTable = null;
    try {
        $sessionsTable = \Illuminate\Support\Facades\Schema::hasTable('sessions') ? 'YES' : 'NO';
    } catch (\Throwable $e) {
        $sessionsTable = 'DB ERROR: ' . $e->getMessage();
    }

    // Round-trip test: did the cookie we set on a previous load come back?
    ...
    return response()->json([
        ...
        'app_key_set'         => config('app.key') ? 'YES' : 'NO (this alone breaks cookies)',
        ...
        'session_dir_exists'  => is_dir($sessionPath) ? 'YES' : 'NO',
        'session_dir_write'   => is_writable($sessionPath) ? 'YES' : 'NO',
        'config_is_cached'    => file_exists(base_path('bootstrap/cache/config.php')) ? 'YES' : 'NO',
        'routes_are_cached'   => file_exists(base_path('bootstrap/cache/routes-v7.php')) ? 'YES' : 'NO',
        'cookies_received'    => array_keys($request->cookies->all()),
        'visit_count'         => $seen + 1,
        'verdict'             => $seen > 0
            ? 'SESSION WORKS — the cookie came back.'
            : 'Reload this page once. If visit_count stays 1, the cookie is NOT persisting.',
    ], 200, ['X-Diag-Probe' => 'alive'], JSON_PRETTY_PRINT);
});
