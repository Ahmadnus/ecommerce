...
@push('head')
<style>
:root{--admin-navy:#0f172a;--admin-navy-light:#1e293b;--admin-accent:#38bdf8}
.admin-auth-bg{background:radial-gradient(circle at top right,#1e293b,#0f172a)}
.input-field{transition:all .2s;background:#f8fafc}
.input-field:focus{background:#fff;border-color:var(--admin-accent);box-shadow:0 0 0 4px rgba(56,189,248,.1)}
@keyframes adminCardIn{from{opacity:0;transform:translateY(15px)}to{opacity:1;transform:translateY(0)}}
.admin-card{animation:adminCardIn .5s ease-out forwards}
.admin-btn{background:var(--admin-navy);transition:all .3s}
.admin-btn:hover{background:var(--admin-navy-light);box-shadow:0 10px 15px -3px rgba(0,0,0,.3)}
.admin-card .iti__tel-input{background:#f8fafc;border-color:#cbd5e1;color:#0f172a}
.admin-card .iti__tel-input:focus{background:#fff;border-color:var(--admin-accent);box-shadow:0 0 0 4px rgba(56,189,248,.1)}
.admin-card .iti--separate-dial-code .iti__selected-flag{border-right-color:#cbd5e1}
</style>
@endpush
    ...
    <div class="w-full max-w-md relative z-10">
        ...
        <div class="text-center mb-8">
            <div class="inline-block p-4 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 mb-4">
                <img src="{{ $logoUrl ?? asset('images/logo.png') }}" alt="Logo" class="h-12 w-auto object-contain brightness-0 invert">
            </div>
            <h2 class="text-white font-display text-2xl font-bold tracking-tight">بوابة الإدارة</h2>
            <p class="text-slate-400 text-sm mt-2">المسؤولون فقط — سجل دخولك</p>
        </div>
        ...
        <div class="admin-card bg-white rounded-3xl shadow-2xl border border-slate-200 p-8 sm:p-10">
            ...
            <form action="{{ route('admin.login') }}" method="POST" class="space-y-6">
                @csrf
                {{-- Mark this as admin login so the controller sets the right OTP intent --}}
                <input type="hidden" name="is_admin_login" value="1">

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">رقم هاتف المسؤول</label>
                ...
                <p class="text-center text-xs text-slate-400 flex items-center justify-center gap-1">
                    ...
                    رمز التحقق سيُرسل لهاتفك بعد التحقق من البيانات
                </p>
            </form>
