...
@section('title', 'تعديل: ' . $homeSection->title)
        ...
        <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 space-y-5">
            ...
            <div>
                ...
                <select name="type" x-model="type" required
                    ...
                    <option value="{{ $value }}"
                            {{ old('type', $homeSection->type) === $value ? 'selected' : '' }}>
                        {{ $label }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="limit"
                           value="{{ old('limit', $homeSection->limit) }}"
                           min="1" max="50"
                           class="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 text-sm
                                  focus:bg-white focus:outline-none focus:ring-2 focus:border-brand transition-all">
                </div>
                <div>
                    ...
                    <input type="number" name="sort_order"
                           value="{{ old('sort_order', $homeSection->sort_order) }}"
                           min="0"
                           class="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 text-sm
                                  focus:bg-white focus:outline-none focus:ring-2 focus:border-brand transition-all">
                </div>
