...
@section('title', 'تعديل: ' . $homeSection->getTranslation('title', app()->getLocale()))
...
<div class="max-w-xl mx-auto">
    ...
    <form action="{{ route('admin.home-sections.update', $homeSection) }}" method="POST" class="space-y-5"
          x-data="{ type: '{{ old('type', $homeSection->type) }}' }">
        @csrf 
        @method('PUT')
        ...
        <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title[ar]" 
                       value="{{ old('title.ar', $homeSection->getTranslation('title', 'ar')) }}"
            ...
            <div>
                ...
                <input type="text" name="title[en]" 
                       value="{{ old('title.en', $homeSection->getTranslation('title', 'en')) }}"
            ...
            <div>
                ...
                <select name="type" x-model="type" required
                    ...
                    <option value="{{ $value }}" {{ old('type', $homeSection->type) === $value ? 'selected' : '' }}>
                        {{ $label }}
                    </option>
                    @endforeach
                </select>
            ...
            <div x-show="type === 'category'" x-transition>
                ...
                <select name="category_id"
                    ...
                    <option value="{{ $cat->id }}" 
                            {{ old('category_id', $homeSection->category_id) == $cat->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="limit" value="{{ old('limit', $homeSection->limit) }}"
                           min="1" max="50"
                           class="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 text-sm
                                  focus:bg-white focus:outline-none focus:ring-2 focus:border-brand transition-all">
                </div>
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $homeSection->sort_order) }}"
                           min="0"
                           class="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 text-sm
                                  focus:bg-white focus:outline-none focus:ring-2 focus:border-brand transition-all">
                </div>
            ...
            <label class="flex items-center gap-3 cursor-pointer p-3 border border-gray-200 rounded-xl bg-gray-50 hover:bg-white transition-colors">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">يظهر على الصفحة الرئيسية عند التفعيل</p>
                </div>
            </label>
        </div>
