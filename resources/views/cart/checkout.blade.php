{{--
    ...
    All $summary values are in JOD. Prices displayed in active currency.
...
<style>
    ...
    .field:focus { background: #fff; border-color: var(--brand-color, #0ea5e9); box-shadow: 0 0 0 3px color-mix(in srgb, var(--brand-color, #0ea5e9) 12%, transparent); }
    .field.has-error { border-color: #ef4444; background: #fef9f9; }
    .step-n { width: 26px; height: 26px; border-radius: 50%; background: #1a1917; color: #fff; font-size: 11px; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
@php
    $cur  = $activeCurrency;                   // set by middleware, defaults to JOD
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
    ...
    <div class="u1 mb-8">
        ...
        <p class="text-xs text-[#9a9793] mt-1">الأسعار معروضة بـ {{ $cur->name }} ({{ $sym }})</p>
    </div>
        ...
        <div class="lg:col-span-3 space-y-5">
            ...
            <div class="u2 bg-white rounded-2xl border border-[#ece9e4] overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-[#f0ede8]">
                    ...
                    <a href="{{ route('cart.index') }}" class="text-xs font-medium text-[#9a9793] hover:text-[#1a1917] transition-colors">تعديل</a>
                </div>
            ...
            <div class="u3 bg-white rounded-2xl border border-[#ece9e4] overflow-hidden">
                ...
                <div class="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold text-[#6b6966] mb-2 uppercase tracking-wide">الاسم الكامل <span class="text-red-400 normal-case">*</span></label>
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold text-[#6b6966] mb-2 uppercase tracking-wide">رقم الهاتف <span class="text-red-400 normal-case">*</span></label>
                        <input type="tel" name="shipping_phone" value="{{ old('shipping_phone', $user->phone ?? '') }}"
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold text-[#6b6966] mb-2 uppercase tracking-wide">العنوان <span class="text-red-400 normal-case">*</span></label>
                        <input type="text" name="shipping_address" value="{{ old('shipping_address') }}"
                               required placeholder="الشارع، الحي..."
                    ...
                    <div>
                        <label class="block text-xs font-bold text-[#6b6966] mb-2 uppercase tracking-wide">المدينة <span class="text-red-400 normal-case">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-[#6b6966] mb-2 uppercase tracking-wide">الرمز البريدي</label>
                        <input type="text" name="shipping_zip" value="{{ old('shipping_zip') }}"
                               placeholder="11118" dir="ltr" class="field">
                    </div>

                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold text-[#6b6966] mb-2 uppercase tracking-wide">ملاحظات</label>
                        <textarea name="notes" rows="2" class="field resize-none" placeholder="تعليمات خاصة...">{{ old('notes') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="u4 bg-white rounded-2xl border border-[#ece9e4] overflow-hidden">
                ...
                <div class="p-5">
                    <label class="flex items-center gap-4 p-4 border-2 border-[#1a1917] rounded-xl cursor-pointer bg-[#faf9f7]">
                        <input type="radio" name="payment_method" value="cod" checked class="w-4 h-4 accent-[#1a1917]">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-[#9a9793] mt-0.5">ادفع نقداً بالدينار الأردني عند وصول طلبك</p>
                        </div>
                ...
                <div class="p-5">
                    ...
                    <div class="space-y-2.5 text-xs border-t border-[#f0ede8] pt-4 mb-4">
                        ...
                        <div class="flex justify-between text-[#9a9793]">
                            ...
                            <span class="font-semibold text-[#1a1917] tabular-nums">{{ $cv($summary['tax']) }} {{ $sym }}</span>
                        </div>
                        <div class="flex justify-between text-[#9a9793]">
                            <span>الشحن</span>
                            @if($summary['shipping'] == 0)
                            <span class="font-semibold text-emerald-600">مجاني</span>
                            @else
                            <span class="font-semibold text-[#1a1917] tabular-nums">{{ $cv($summary['shipping']) }} {{ $sym }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <button type="submit" id="place-btn"
                            class="w-full py-4 rounded-xl bg-[#1a1917] hover:bg-[#2d2c2a] text-white
                                   font-bold text-sm tracking-wide transition-colors flex items-center
                                   justify-center gap-2 shadow-lg shadow-black/15 active:scale-[.98]">
