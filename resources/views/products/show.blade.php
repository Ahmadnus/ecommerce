...
/* ─── Price ───────────────────────────────────────────────────────── */
.price-sale   { color: var(--text-price, #ff3366); }
.price-normal { color: var(--text-price, #111827); }
...
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-20">
        ...
        <div class="flex flex-col">
            ...
            {{-- ✅ Star rating summary (shows only if reviews exist) --}}
            @php
            ...
            <div class="flex items-center gap-2 mb-4">
                ...
                <span class="text-sm font-bold" style="color:var(--text-heading,#0f172a);">
...
function resolveVariant() {
    ...
    if (stockEl && match) {
        stockEl.innerHTML = match.stock > 0
            ? '<div class="w-2.5 h-2.5 rounded-full bg-emerald-500"></div><span class="text-sm text-emerald-700 font-medium">' + I18N.inStock + ' (' + match.stock + ')</span>'
            : '<div class="w-2.5 h-2.5 rounded-full bg-red-400"></div><span class="text-sm text-red-600 font-medium">' + I18N.outOfStock + '</span>';
    }
...
function validateSelections() {
    ...
    required.forEach(function (attrId) {
        if (typeof window.SEL_AVS[attrId] === 'undefined') { markAttrError(attrId); missingCount++; }
        else { clearAttrError(attrId); }
    });
    var banner  = document.getElementById('cart-error-banner');
    var errText = document.getElementById('cart-error-text');
    if (missingCount > 0) {
        if (errText) errText.textContent = I18N.cartErrorCount.replace(':count', missingCount);
...
function addToCart() {
    ...
    var payload = { product_id: "{{ $product->id }}", quantity: qty };
    if (selectedVariant && selectedVariant.id) payload.variant_id = selectedVariant.id;
